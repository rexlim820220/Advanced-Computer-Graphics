...
{
    ifstream ifile("Input_Bunny.txt");
    ...
    ofstream ofile("Output_Bunny.ppm",std::ios::binary);
    ofile<<"P6"<<'\n'<<width<<' '<<height<<' '<<"\n255\n";
            ...
            for(size_t k=0;k<triangle.size();k+=1){
                ...
                if(triangle[k].Moller_Trumbore(light,vec,t)){
                    ...
                    if(temp>t){
                        ...
                        ambient=P.ka;
                        diffuse=P.kd*dot_product(N,ray);//all positive
                        specular=P.ks*pow(dot_product(N,L),P.exp);//all positive
                        temp=t;
                    }
                }
            }
